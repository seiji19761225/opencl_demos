...
int main(int argc, char **argv)
{
    ...
    dev_u     = clCreateBuffer(context, CL_MEM_READ_WRITE,
				STRIDE0*STRIDE1*DEPTH*sizeof(real_t), NULL, NULL);
    ...
    // CPU->GPU memory copy
    clEnqueueWriteBuffer(queue, dev_m, CL_FALSE, 0, 9*sizeof(real_t), m, 0, NULL, NULL);
...
{				/* setup initial status.                */
    ...
    if ((u = MALLOC(STRIDE0*STRIDE1*DEPTH, real_t)) == NULL ||
	(v = MALLOC(STRIDE0*STRIDE1*DEPTH, real_t)) == NULL) {
	perror("malloc");
	exit(EXIT_FAILURE);
    }
...
#pragma omp parallel for private(i,j,k) collapse(3)
    ...
    clEnqueueWriteBuffer(queue, dev_u, CL_TRUE, 0,
			 STRIDE0*STRIDE1*DEPTH*sizeof(real_t), u, 0, NULL, NULL);
    clEnqueueWriteBuffer(queue, dev_v, CL_TRUE, 0,
			 STRIDE0*STRIDE1*DEPTH*sizeof(real_t), v, 0, NULL, NULL);
...
{				/* update status.                       */
...
#ifdef USE_LOCAL_MEMORY
    size_t global_size[] = { ROUND_UP(width,LW), ROUND_UP(height,LH), ROUND_UP(depth,LD) };
    size_t  local_size[] = { LW, LH, LD };
#else
    size_t global_size[] = { ROUND_UP(width,32), ROUND_UP(height,32), ROUND_UP(depth,32) };
    ...
    clSetKernelArg(kernel_fdm, i++, sizeof(int   ), &depth );
    ...
    clSetKernelArg(kernel_fdm, i++, 2*STRIDE2*STRIDE3*(LD+2)*
				    sizeof(real_t),    NULL);
#endif
    ...
    clSetKernelArg(kernel_euler, i++, sizeof(int   ), &depth );
...
{				/* update graphics on the window.       */
    ...
    clSetKernelArg(kernel_update_image, i++, sizeof(int   ), &depth    );
