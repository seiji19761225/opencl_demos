...
int main(int argc, char **argv)
{
    ...
    if ((u = MALLOC(WIDTH*HEIGHT, real_t)) == NULL ||
	(v = MALLOC(WIDTH*HEIGHT, real_t)) == NULL ||
	(p = MALLOC(WIDTH*HEIGHT, real_t)) == NULL ||
	(q = MALLOC(WIDTH*HEIGHT, real_t)) == NULL) {
	perror("malloc");
	return EXIT_FAILURE;
    }
...
#pragma omp parallel private(t,ii,jj)
    for (t = 0; t < TI; t++) {
