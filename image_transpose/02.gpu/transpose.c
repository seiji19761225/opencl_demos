 ...
 * $Id: transpose.c,v 1.1.1.2 2021/07/18 00:00:00 seiji Exp seiji $
 */
...
void   transpose (cl_obj_t *, cl_kernel   , int  , int  , cl_mem, cl_mem);
...
//	"-DUSE_REORDERING "
    ...
    ts = wtime();
    transpose(obj, kernel, width, height, dev_i, dev_o);
    te = wtime();
    ...
    save_image(obj, OUTPUT_FILE, height, width, dev_o);
...
//----------------------------------------------------------------------
void transpose
	(cl_obj_t *obj, cl_kernel kernel, int width, int height,
	 cl_mem dev_input, cl_mem dev_output)
{
    ...
    clSetKernelArg(kernel, i++, (LH+1)*LW*sizeof(uchar), NULL);
#endif
